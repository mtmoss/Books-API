...
route_books = APIRouter(prefix="/api/v1/books", tags=["books"])

@route_books.get("/", response_model=OutputBooks)
def list_all_books():
    items = list_books()
    return {"total": len(items), "items": items}

@route_books.get("/search", response_model=OutputBooks)
def search_books(
...
@route_books.get("/categories", response_model=List[str])
def get_categories():
    return list_categories()

@route_books.get("/{id}", response_model=Book)
